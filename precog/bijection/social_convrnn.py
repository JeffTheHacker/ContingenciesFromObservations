...
class SocialConvRNN(esp_bijection.ESPJointTrajectoryBijectionMixin, interface.ESPJointTrajectoryBijection):
    ...
    def __init__(self,
        ...
        self.final_layer = [tf.compat.v1.layers.AveragePooling2D(pool_size=(3, 3),strides = (3,3)),
                            tf.compat.v1.layers.Flatten(),
                            tf.compat.v1.layers.Dense(units = 50)]
    ...
    def variables(self):
        ...
        # add the convnet + my dense layer to the trainable variables
        vars_ += tensoru.layer_variables(self.convnet)
        vars_ += tensoru.layer_variables(self.final_layer)
    ...
    def _prepare(self, batch_shape, phi):
        ...
        if self.feature_map is None:
            if self.cnnconf.create_residual_connections:
                ...
                # use the feature map to generate a feature vector of dimension 50
                Pool = self.final_layer[0]
                Flatten = self.final_layer[1]
                Dense = self.final_layer[2]
                self.feature_vec = Dense(Flatten(Pool(self.feature_map)))
                # output shape is (batch size, 50)
    ...
    def step_generate(self, S_history, phi, *args, **kwargs):
        ...
        S_tm1_grid = tf.identity(S_tm1)
        S_tm1_world = phi.local2world.apply(S_tm1, self.batch_str + 'aj')

        # used for social velocity
        S_tm2 = S_history[-2]
        S_tm2_grid = tf.identity(S_tm2)
        S_tm2_world = phi.local2world.apply(S_tm2, self.batch_str + 'aj')

        S_tm3 = S_history[-3]
        S_tm3_grid = tf.identity(S_tm3)
        S_tm3_world = phi.local2world.apply(S_tm3, self.batch_str + 'aj')
        ...
        second_last_agent_position_feats = bijection_helpers.get_social_spatial_differences(
            phi=phi, batch_shape=self.batch_shape, batch_size=self.batch_size, last_agent_positions_world=S_tm2_world, A=self.A)

        third_last_agent_position_feats = bijection_helpers.get_social_spatial_differences(
            phi=phi, batch_shape=self.batch_shape, batch_size=self.batch_size, last_agent_positions_world=S_tm3_world, A=self.A)
        ...
        if self.socialconf.use_social_feats:
            ...
            # Some extra featurization in time of social feats
            social_velocity = last_agent_position_feats - second_last_agent_position_feats
            # acceleration
            social_acceleration = social_velocity - (second_last_agent_position_feats - third_last_agent_position_feats)
            # Combine the social feats together
            social_mlp_input = tf.concat((last_agent_position_feats, second_last_agent_position_feats, third_last_agent_position_feats, social_velocity, social_acceleration), axis=-1)
            # Pass the social feats through the MLP
            social_feats_all = tf.reshape(tensoru.mlp(self.social_mlp, social_mlp_input), (self.batch_size, self.A, self.G))
            social_feats_A_batch = tf.reshape(social_feats_all, (self.A_batch_size, self.G))
        ...
        #self.feature_vec_A_batch = tf.concat([self.feature_vec for i in range(self.batch_shape[-1] * self.A)],axis = 0) # build the feature vector batch
        self.feature_vec_A_batch =  tf.tile(self.feature_vec,[self.batch_shape[-1] * self.A,1])

        # Build the input to the RNN.
        joint_feat_tup = (self.past_encodings_A_batch, self.yaws_A_batch, self_positions_feat_A_batch, self.feature_vec_A_batch)
        if self.lightconf.use_light_feats: # true
        ...
        if self.socialconf.use_social_feats: # True
            ldb("Using social feats")
            joint_feat_tup += (social_feats_A_batch,)
        # else:
        #     assert self.socialconf.use_social_feats == True
        if self.socialconf.use_social_map_feats:
            ldb("Using social feats")
            joint_feat_tup += (social_map_feats_A_batch,)
        if self.cnnconf.create_overhead_feature: # false
