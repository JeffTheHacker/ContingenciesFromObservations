...
def plot_joint_trajectory(joint_traj, key='joint_trajectories', fig=None, ax=None, render=True, limit=100, **kwargs):
    ...
    for a in range(A):
        ...
        if tensoru.size(joint_traj, 0) == 1: # my modification
            color = COLORS[-2]
...
def plot_bev(sampled_output, batch_index, ax, onechannel=True, fmt='carla', allchannel=False, channel_idx=0):
    if onechannel:
        ...
        # my mod: recover the lidar points, then transform to 2D representation
        def range_map_to_point_cloud(range_map):
            ...
            for i,x in enumerate(range_map):
                for j,y in enumerate(x):
                    ...
                    #result.append([cor_x + 200,cor_y - 165,cor_z])
                    result.append([cor_x,cor_y])
            return result
        # local = range_map_to_point_cloud(bev)
        # yaw = np.deg2rad(sampled_output.phi.yaws[batch_index][0] + 90)
        # origin = sampled_output.phi.S_past_world_frame[batch_index][0][-1]
        # R = np.array([[np.cos(yaw),-np.sin(yaw)],[np.sin(yaw),np.cos(yaw)]])
        # world = np.dot(R,np.array(local).T).T + origin
        # X = world[:,0].tolist()
        # Y = world[:,1].tolist()
        #
        # ax.plot(X,Y,linestyle = 'None',marker = 'o',color='r',markersize=0.6,alpha=0.4)
        # print("Finished plotting lidar.")
...
def plot_expert(expert, batch_index, fig, ax, limit, render=False, **kwargs):
    ...
    return plot_joint_trajectory(expert.S_future_world_frame[batch_index][None], # used to be S_future_grid_frame
                                 key='expert',
                                 render=render,
                                 fig=fig,
                                 ax=ax,
...
def plot_sampled_minibatch(sampled_output,
    ...
    expert_kwargs = {'color': COLORS[1]} if A == 1 else {'color': COLORS[-2]}
    for b in range(min(B,10)):
        ax = axes.ravel()[b]
        magic_reset_axis(ax)
        # commented the below out becaue it does same thing as plot_rollout
        ...
        plot_past(sampled_output.phi.S_past_world_frame, b=b, fig=fig, ax=ax, limit=limit)
        ...
        # ax.set_ylim(-60,10)
        # ax.invert_xaxis()
        x_cors = experts.S_future_world_frame[b][0][:,0]
        y_cors = experts.S_future_world_frame[b][0][:,1]
        ax.set_xlim(min(x_cors) - 15,max(x_cors) + 15)
        ax.set_ylim(min(y_cors) - 15,max(y_cors) + 15)
        ax.invert_xaxis()

        # if -112 <= sampled_output.phi.S_past_world_frame[b][0][0][0] <= -62:
        #     # for overtake scenario:
        #     print("no..")
        #     ax.set_xlim(-112,-62)
        #     ax.set_ylim(-60,10)
        #     ax.invert_xaxis()
...
def plot_rollout(rollout, b=None, fig=None, ax=None):
    ...
    limit = tensoru.size(rollout.phi.overhead_features, 1)
    plot_joint_trajectory(joint_traj=rollout.S_world_frame[b], # used to be S_grid_frame
                          key='rollout_future',
                          render=False,
                          fig=fig,
                          ax=ax,
