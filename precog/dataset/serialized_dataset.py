...
class SerializedDataset(interface.ESPDataset, minibatched_dataset.MinibatchedDataset):
    ...
    def get_minibatch(self, is_training, split='train', mb_idx=None, input_singleton=None, *args, **kwargs):
        ...
        if self.Other_count > 0:
            ...
            other_experts = np.stack([np.asarray(k(_, 'agent_futures')) for _ in data], 0)[None]
            # (O, B, Tp, d)
            ...
            other_pasts = np.stack([np.asarray(k(_, 'agent_pasts')) for _ in data], 0)[None]
            # (O, B)
            other_yaws = np.stack([npu.fill_axis_to_size(
                k(_, 'agent_yaws')[None][:self.Other_count], axis=0, size=self.Other_count) for _ in data], 1)
        ...
        if self.load_bev:
            ...
            #bevs = npu.batch_center_crop(bevs, target_h=self.W, target_w=self.W) # no need to crop
            bevs = np.expand_dims(bevs,axis = 3) # my modification
